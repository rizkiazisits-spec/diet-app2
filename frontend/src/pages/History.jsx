...
export default function History() {
  ...
  const totalCalories = Math.round(items.reduce((s, i) => s + (i.kalori || i.kalori_terbakar || 0), 0));

  const secondStatLabel = tab === 'food' ? 'TOTAL ITEM' : 'DURASI OLAHRAGA';
  const secondStatValue = tab === 'food'
    ? `${items.length} porsi`
    : `${items.reduce((s, i) => s + (i.durasi_menit || 0), 0)}m`;

  const secondStatSub = tab === 'food'
    ? `${items.length} Makanan dicatat`
    : `${items.length} Sesi Olahraga`;
      ...
      <main className="p-5 space-y-5">
        ...
        <section className="space-y-3">
          {loading ? (
            ...
            <p className="text-slate-400 dark:text-neutral-500 text-center py-6 text-xs">
              {tab === 'food' ? 'Belum ada riwayat makanan.' : 'Belum ada riwayat olahraga.'}
            </p>
          ) : (
        ...
        <section className="bg-[#0f172a] text-white p-6 rounded-2xl border border-slate-800 shadow-md flex flex-col gap-3 relative overflow-hidden">
          ...
          <p className="text-[11px] text-slate-300 leading-relaxed max-w-[90%]">
            Catat makanan dan olahraga harianmu untuk memantau asupan dan pembakaran kalori secara akurat.
          </p>
