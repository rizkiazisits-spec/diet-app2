  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = typeof detail === 'string'
        ? detail
        : (Array.isArray(detail) ? detail.map((d) => d.msg).join(', ') : 'Login gagal. Silakan periksa kredensial Anda.');
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="bg-[#f0f4fa] dark:bg-[#0a0a0a] min-h-screen flex items-center justify-center p-5 font-sans text-[#171c20] dark:text-[#f8fafc] transition-colors duration-200 relative">
      ...
      <main className="w-full max-w-[420px] relative z-10">
        {/* Logo and Intro (Image 6) */}
        <div className="text-center mb-8 flex flex-col items-center">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-[#0ea5e9] text-white mb-4 shadow-md">
          ...
          <p className="text-xs text-slate-500 dark:text-neutral-400 font-semibold max-w-[280px] mt-1.5 leading-relaxed">
            Precision diet management for your clinical health goals.
          </p>
        </div>
        ...
        <div className="glass-card bg-white dark:bg-[#1e1e1e] p-8 w-full border border-slate-100 dark:border-neutral-800 rounded-3xl shadow-lg">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-neutral-400 block" htmlFor="email">Email Address</label>
              <div className="relative">
                ...
                <input className="input-premium w-full h-11 pl-11 pr-4 text-xs font-semibold bg-[#f0f4fa] dark:bg-neutral-800 border-none"
                  id="email" type="email" placeholder="name@example.com" required value={email} onChange={(e) => setEmail(e.target.value)} />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                <label className="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-neutral-400 block" htmlFor="password">Password</label>
                <Link to="#" className="text-[11px] font-bold text-[#0ea5e9] hover:underline">Forgot?</Link>
              </div>
              <div className="relative">
                ...
                <input className="input-premium w-full h-11 pl-11 pr-12 text-xs font-semibold bg-[#f0f4fa] dark:bg-neutral-800 border-none"
                  id="password" type={showPw ? 'text' : 'password'} placeholder="••••••••" required value={password} onChange={(e) => setPassword(e.target.value)} />
            ...
            <button type="submit" disabled={loading}
              className="w-full h-12 mt-2 rounded-full bg-[#0ea5e9] text-white text-xs font-bold tracking-wide flex items-center justify-center gap-2 shadow-lg shadow-sky-500/25 hover:bg-[#0284c7] hover:scale-[1.01] active:scale-[0.99] transition-all disabled:opacity-60 cursor-pointer">
          ...
          <div className="grid grid-cols-2 gap-3.5">
            {/* Google */}
            <button type="button" className="flex items-center justify-center h-11 border border-slate-200 dark:border-neutral-800 hover:bg-slate-50 dark:hover:bg-neutral-800 rounded-xl transition-all cursor-pointer font-bold text-xs">
            ...
            {/* Apple */}
            <button type="button" className="flex items-center justify-center h-11 border border-slate-200 dark:border-neutral-800 hover:bg-slate-50 dark:hover:bg-neutral-800 rounded-xl transition-all cursor-pointer font-bold text-xs">
              <svg className="w-4 h-4 mr-2 fill-current text-slate-800 dark:text-neutral-200" viewBox="0 0 24 24">
        ...
        {/* Bottom Switch */}
        <p className="text-center mt-6 text-xs text-slate-500 dark:text-neutral-400 font-semibold">
          Belum punya akun?{' '}
          <Link to="/register" className="font-bold text-[#0ea5e9] hover:underline ml-1">Daftar</Link>
        </p>
      </main>
    </div>
  );
